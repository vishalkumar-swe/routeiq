      ...
      <div className="w-full bg-surface border-y border-primary/30 overflow-hidden py-4 flex items-center shadow-2xl relative z-20">
        ...
        <div className="flex animate-infinite-scroll whitespace-nowrap gap-16 items-center px-12">
          {tickerAlerts.concat(tickerAlerts).map((alert: string, i: number) => (
            <span key={i} className="text-white/90 text-xs font-bold tracking-tight flex items-center gap-4">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-6">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2 rounded-full bg-surface2 text-white text-[10px] font-black tracking-[0.1em] uppercase border border-border hover:border-primary transition-all disabled:opacity-50"
            >
          ...
          <h1 className="text-7xl font-black text-white font-display tracking-tighter uppercase mb-6 leading-none">
      ...
      <div className="rounded-[3rem] bg-surface border border-border p-10 shadow-2xl">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-black text-white font-display tracking-tight uppercase">Decision Stream</h2>
              <p className="text-muted text-[10px] font-bold tracking-[0.2em] uppercase mt-1">Live Heuristic Logs</p>
            </div>
          </div>
          <button className="px-8 py-3 rounded-2xl bg-surface2 border border-border text-muted text-xs font-black hover:text-white hover:bg-bg transition-all uppercase tracking-widest">
            Export Model Data
          </button>
        </div>
          ...
          {insights.length > 0 ? insights.map((log: any, i: number) => (
            <div key={i} className="flex items-center justify-between p-5 rounded-[24px] bg-surface2/40 border border-border group hover:border-primary/50 transition-all cursor-pointer">
              <div className="flex items-center gap-6">
                ...
                <span className="text-muted font-bold text-sm tracking-tight group-hover:text-white transition-colors">{log.insight}</span>
              </div>
    ...
    <div className={clsx(
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <div className="text-base font-black text-white uppercase tracking-tight">{mission.plate_number}</div>
              {mission.last_sync && (
        ...
        <div className="text-right">
          <div className={clsx(
            ...
            isOptimizable ? "text-primary" : "text-white"
          )}>
            {mission.ai_efficiency_score.toFixed(1)}%
          </div>
          <div className="text-[9px] font-black text-muted uppercase tracking-widest mt-1">AI Efficiency</div>
        </div>
      </div>
      ...
      <div className="space-y-6 relative z-10">
        <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-[0.2em]">
          ...
          <span className="text-white">{Math.round(mission.progress_pct)}%</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <div className="p-4 rounded-2xl bg-surface2/50 border border-border">
            <div className="text-[9px] font-black text-muted uppercase tracking-widest mb-1.5">Speed</div>
            <div className="text-base font-black text-white">{mission.speed} <span className="text-[10px] text-muted ml-1">KM/H</span></div>
          </div>
          <div className={clsx(
        ...
        <button 
          ...
          className={clsx(
            ...
            isOptimizable 
              ...
              : "bg-surface2 text-white border-border hover:border-primary/40 disabled:opacity-50"
          )}
        >
