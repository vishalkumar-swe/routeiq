      ...
      <div className="grid grid-cols-1 gap-4">
        {isLoading ? (
          <div className="py-40 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] animate-pulse">Syncing Cargo Manifest...</p>
          </div>
        ) : shipments.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-20 text-center bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
             <Package className="text-slate-300 mb-6" size={64} />
             <h3 className="text-xl font-bold text-slate-900 mb-2 uppercase tracking-tighter">No Active Neural Loads</h3>
             <p className="text-sm text-slate-400 max-w-xs font-bold leading-relaxed">Initialize your first shipment tracking vector to begin logistics optimization.</p>
          </div>
        ) : (
          filtered.map((s: any) => (
...
function AddShipmentModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 backdrop-blur-2xl bg-slate-950/40 animate-in fade-in duration-500">
      <div className="bg-white rounded-[3rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] border border-white/20 animate-in zoom-in-95 duration-500 scrollbar-none">
        ...
        <div className="relative p-10 bg-slate-900 overflow-hidden">
           ...
           <div className="relative z-10">
             ...
             <h2 className="text-4xl font-black text-white font-display tracking-tight uppercase leading-none mb-2">Request Shipment</h2>
             <p className="text-slate-400 font-bold text-sm tracking-tight">Deploying cargo onto the active pan-India logistics network.</p>
           </div>
        ...
        <div className="p-10 space-y-10">
            ...
            <div className="space-y-4">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full h-14 pl-14 pr-6 bg-slate-50 border-2 border-transparent rounded-2xl text-slate-900 font-bold placeholder:text-slate-400 focus:bg-white focus:border-blue-500/30 transition-all outline-none text-sm"
                />
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 z-[60] bg-white rounded-2xl border border-slate-100 shadow-2xl overflow-hidden">
                    {originSuggestions.map((p: any) => (
                      ...
                      >
                        ...
                        <span className="text-[9px] font-bold text-slate-400 uppercase truncate">{p.place_name}</span>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <div className="space-y-4">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full h-14 pl-14 pr-6 bg-slate-50 border-2 border-transparent rounded-2xl text-slate-900 font-bold placeholder:text-slate-400 focus:bg-white focus:border-yellow-500/30 transition-all outline-none text-sm"
                />
          ...
          <div className="space-y-4">
             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Cargo Payload Configuration</label>
             <div className="flex gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                     <div className="relative">
                        <Thermometer size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                        <input 
                          type="number"
          ...
          <div className="space-y-4">
             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1 flex items-center gap-2">
               <Layers size={12} className="text-slate-400" /> Shipment Intelligence
             </label>
                  ...
                  >
                    <div className="flex items-center gap-4 relative z-10">
                       <div className={clsx(
                         ...
                         formData.cargo_type === cargo.id ? "bg-white/10" : "bg-slate-50"
                       )}>
                       ...
                       <div>
                          ...
                          <div className={clsx(
                            "text-[10px] font-bold",
                            formData.cargo_type === cargo.id ? "text-slate-400" : "text-slate-500"
                          )}>{cargo.desc}</div>
                       </div>
                    </div>
                    {formData.cargo_type === cargo.id && (
                       <Zap size={40} className="absolute -right-4 -bottom-4 text-white/5 rotate-12" />
                    )}
                  </button>
                ))}
             </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="space-y-4">
               <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Priority Grid</label>
                  ...
                  {PRIORITIES.map(p => (
                    <button
                      ...
                      className={clsx(
                        ...
                        formData.priority === p ? "bg-slate-900 text-yellow-400" : "bg-slate-50 text-slate-400 hover:bg-slate-100"
                      )}
                    >
                      {p}
                    </button>
             ...
             <div className="space-y-4">
                <div className="flex items-center justify-between">
                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Plan For Later</label>
                   <button 
                ...
                {formData.plan_for_later && (
                  <div className="flex gap-2 animate-in slide-in-from-right-4 duration-300">
                     <div className="relative flex-1 group">
                        <Calendar size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-yellow-500" />
                        <input 
                           type="date"
                     ...
                     <div className="relative flex-1 group">
                        <Clock size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-yellow-500" />
                        <input 
                           type="time"
                           className="w-full h-12 pl-10 pr-3 bg-slate-50 border-none rounded-xl text-[10px] font-bold text-slate-900 focus:ring-2 focus:ring-yellow-500/20"
